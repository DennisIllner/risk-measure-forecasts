...
#Model fit AR(1)-GARCH(1,1)-N

#specifying model
spec<-ugarchspec(variance.model =list(model = "sGARCH", garchOrder = c(1,1)) , # GARCH(1,1)
                 mean.model = list(include.mean = FALSE,armaOrder = c(1,0)),   # AR(1)
...
q_sgarch.t <- preds$Mu + preds$Sigma  * f(alpha,preds) #VaR forecasts
...
ES <- preds$Mu + preds$Sigma * integrate(f, 0, 0.01)$value / 0.01 # ES forecasts 
